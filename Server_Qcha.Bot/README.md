...
1. 复制配置文件并按需修改：
   - `src/Server.Qcat/appsettings.Example.json` -> `src/Server.Qcat/appsettings.Local.json`
...
主要配置在 `src/Server.Qcat/appsettings.json`，推荐只在 `src/Server.Qcat/appsettings.Local.json` 放本机配置（已在 `.gitignore` 里忽略）。
